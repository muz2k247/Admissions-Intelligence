...
description: Extract deadline, fee, programs, and constituent_college fields from a chunk of scraped admissions content, under a strict null-if-unstated / never-guess contract. Used as the primary field extractor, replacing the weaker regex-only extraction/fields.py for chunks it covers.
model: sonnet
tools: Read, Write
---
...
4. Write the output JSON file: an object keyed by chunk `id`, each value shaped as `{"deadline": {...}, "fee": {...}, "programs": {...}, "constituent_college": {...}}` (see Output Format).
...
**`fee`** — the application/admission processing fee explicitly stated (e.g. "Application Fee: Rs. 3,000/-"). Not a semester fee, hostel fee, or entry-test registration fee for a different purpose — those are a different fact, not a weaker signal for this one.
